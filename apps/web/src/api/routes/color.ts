...
// Env is globally available from worker-configuration.d.ts (generated by wrangler types)

const colorInputSchema = z
  .object({
    color: z.string().openapi({
      description: "Any CSS color string: hex, rgb, hsl, oklch, named color, etc.",
      example: "#3b82f6",
    }),
  })
  .openapi("ColorInput");
...
const contrastSchema = z
  .object({
    ...
    wcagAANormal: z.boolean().openapi({ description: "Passes WCAG AA for normal text (4.5:1)" }),
    wcagAALarge: z.boolean().openapi({ description: "Passes WCAG AA for large text (3:1)" }),
    wcagAAA: z.boolean().openapi({ description: "Passes WCAG AAA (7:1)" }),
...
const harmonySchema = z
  .object({
    ...
    analogous: z.array(oklchSchema).openapi({ description: "30 degrees each side" }),
    triadic: z.array(oklchSchema).openapi({ description: "120 degree spacing" }),
    splitComplementary: z.array(oklchSchema).openapi({ description: "150/210 degree splits" }),
  })
...
export function registerColorRoutes(app: OpenAPIHono<HonoEnv>) {
